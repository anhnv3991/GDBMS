 ...
 */
__global__ void hashInitialize(GHashIndex table_index, int64_t *table, GColumnInfo *table_schema, int columns, int left, int right) {
 ...
 */
__global__ void hashCount(GHashIndex indexes, ulong *hash_count, int left, int right, uint64_t max_buckets)
{
	int index = threadIdx.x + blockIdx.x * blockDim.x;
	int stride = blockDim.x * gridDim.x;
...
__global__ void bucketsLocate(ulong *hash_count, int *bucket_locations, int bucket_num)
{
	int index = threadIdx.x + blockIdx.x * blockDim.x;
	int stride = blockDim.x * gridDim.x;
...
__global__ void gHash(GHashIndex indexes, ulong *hash_count, int *sorted_idx, int bucket_num, int start_idx, int end_idx)
{
	int index = threadIdx.x + blockIdx.x * blockDim.x;
	int stride = blockDim.x * gridDim.x;
...
__global__ void hashSearchUpper(GHashIndex indexes, int key_left, int key_right, int *output, int *bucket_locations)
{
	int index = threadIdx.x + blockIdx.x * blockDim.x;
	int stride = blockDim.x * gridDim.x;
	GHashIndexKey key;
...
__global__ void hashSearchLower(GHashIndex indexes, int key_left, int key_right, int *output, int *bucket_locations)
{
	int index = threadIdx.x + blockIdx.x * blockDim.x;
	int stride = blockDim.x * gridDim.x;
	GHashIndexKey key;
...
__global__ void constructHashLocation(int *location, int size)
{
	int index = threadIdx.x + blockIdx.x * blockDim.x;
	int stride = blockDim.x * gridDim.x;
...
__global__ void hashArrange(int *input, int *output, int *location, int size) {
	int index = threadIdx.x + blockIdx.x * blockDim.x;
	int stride = blockDim.x * gridDim.x;
